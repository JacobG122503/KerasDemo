...
print(f"Attempting to start TensorBoard on port {TENSORBOARD_PORT}...")
try:
    # Use Popen to run TensorBoard in the background.
    # The `bind_all` flag is important for accessing it from outside the container/machine.
    proc = subprocess.Popen(
        ["tensorboard", "--logdir", LOG_DIR, "--port", str(TENSORBOARD_PORT), "--bind_all"],
        stdout=subprocess.DEVNULL, # Suppress tensorboard's stdout
        stderr=subprocess.STDOUT   # and stderr
    )
    print(f"TensorBoard process started in the background with PID: {proc.pid}")
    print("You can view progress by forwarding the port from the cluster to your local machine.")
except Exception as e:
    print(f"\nCould not start TensorBoard automatically: {e}")
    print("You may need to start it manually on the cluster: tensorboard --logdir logs --bind_all")
...
download_and_extract("https://github.com/jbrownlee/Datasets/releases/download/Flickr8k/Flickr8k_Dataset.zip", "Flickr8k_Dataset.zip")
download_and_extract("https://github.com/jbrownlee/Datasets/releases/download/Flickr8k/Flickr8k_text.zip", "Flickr8k_text.zip")
