...
mkdir -p ~/gpt2_text_generation_with_keras_hub
cd ~/gpt2_text_generation_with_keras_hub
...
run_epochs() {
    ...
    scp "${REMOTE_USER}@${REMOTE_HOST}:~/gpt2_text_generation_with_keras_hub/models/gpt2_keras_hub_${epochs}_epochs.keras" "$LOCAL_DIR/models/" || echo "Failed to download model!"
}
