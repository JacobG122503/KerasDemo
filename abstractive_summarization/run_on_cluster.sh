...
mkdir -p ~/abstractive_summarization
...
run_epochs() {
    ...
    scp "${REMOTE_USER}@${REMOTE_HOST}:~/abstractive_summarization/models/bart_summarization_${epochs}_epochs.keras" "$LOCAL_DIR/models/" || echo "Failed to download model!"
}
