...
mkdir -p ~/text_generation_gpt
cd ~/text_generation_gpt
...
run_epochs() {
    ...
    scp "${REMOTE_USER}@${REMOTE_HOST}:~/text_generation_gpt/models/text_generation_${epochs}_epochs.keras" "$LOCAL_DIR/models/" || echo "Failed to download model!"
}
